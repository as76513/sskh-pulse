...
- `employees` — PK `emp_code`. GSI `email-index` (PK `email`) exists only so `createEmployee` can check email uniqueness before insert — it's a query-then-write check, not a transactional constraint, which is an accepted simplification at this company's scale. GSI `username-index` (PK `username`) resolves the login screen's username (e.g. `john.doe`) to the employee record — see Auth flow below. No `password_hash` field anymore; passwords live in Cognito.
...
**Auth flow**: passwords are NOT stored by this app — they live in an existing, shared Cognito User Pool (`ap-south-1_Gj6XcAi9D`) owned by a different, unrelated app that already has its own real users. This app never modifies that pool's schema/settings, only owns its own app client inside it (`infra/cognito.tf`, `aws_cognito_user_pool_client.sskh_pulse`), created purely additively. The login screen collects a **username** (e.g. `john.doe`), never an email — `authController.login` resolves that to the employee record via the `username-index` GSI, then sends *that record's `email`* + the password to Cognito's `AdminInitiateAuth` (`config/cognito.js`). Cognito is only ever used as a password oracle here: once it confirms the password, this app still signs its **own** JWT (`JWT_SECRET`, `JWT_EXPIRES`) containing `emp_code` + `role`, exactly as before Cognito was introduced — `authRequired`/`me`/every other controller are completely unaware Cognito exists. `changePassword` does the same old-password check via `AdminInitiateAuth`, then `AdminSetUserPassword` for the new one — Cognito's pool policy (8+ chars, upper+lower+number+symbol) applies, not any check in this app's code. `createEmployee` calls `AdminCreateUser`+`AdminSetUserPassword` instead of hashing anything itself. The frontend stores this app's own JWT in `localStorage` (`sskh_token`) and attaches it as `Authorization: Bearer` on every request via `frontend/src/api/client.js`'s `api()` helper — unchanged. No local Cognito emulator exists; local dev hits the real shared pool too (same pattern as S3 — see `backend/.env.example`).
